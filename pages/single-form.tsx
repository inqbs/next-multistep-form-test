  ...
  return (
    <div className='w-6/12 mx-auto'>
      ...
      <form className="shadow m-4" onSubmit={handleSubmit(onSubmit)}>
        <section className='pt-8 pb-6 px-4 mb-3 bg-gradient-to-r from-sky-500 to-indigo-500'>
          <h2 className='font-bold text-4xl text-white'>Album Registeration Form</h2>
         </section>
        ...
        <fieldset className='block w-full mb-4 p-4'>
          ...
          {
            Array(songLength).fill('').map((_, idx) => (
              <div key={`song-${idx}`} className='flex justify-between items-center gap-4'>
                <span className='inline-block font-bold w-3'>{idx + 1}.</span>
                <UnderlineInput type="text" {...register(`song[${idx}].title`, { required: true })} placeholder='Song Title' className='w-[25%]' />
                <UnderlineInput type="text" {...register(`song[${idx}].artist`, { required: true })} placeholder='Artist' />
              </div>
            ))
          }
          {
            (Boolean(((Array)(errors.song)?.some?.(it => it != null) ?? false))) && <p className='text-red-500'>Song info is required</p>
          }
        </fieldset>
